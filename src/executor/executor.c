...
void	execute_last_cmd(t_minishell *mini, t_cmd *current_cmd)
{
	pid_t       pid;
    int         status;
	// int			prev_read_end = STDIN_FILENO; // initialize the read end of the pipe to standard input
...
void    execute_multiple_commands(t_minishell *mini)
{
    ...
    while (current_cmd != NULL) // loop through linked list s_cmd made of t_cmd's:
    {
		tmp_fd_in = dup(0);
		tmp_fd_out = dup(1);
		if (current_cmd->next != NULL) // if current_cmd is not last cmd
		{
			...
			else // parent process
			{
            	...
            	// close(prev_read_end); // close the read end of the previous pipe
           		// prev_read_end = fd_pipe[READ]; // save the read end of the current pipe for the next iteration
				// parent must wait for last command/ child process to finish before printing to shell prompt
				if (waitpid(pid, &status, 0) < 0)
					return (minishell_error("waitpid error"));
			}
			...
			execute_last_cmd(mini, current_cmd);
