...
int	change_directory(t_cmd *cmd, t_env_var_ll **env_var_list)
{
	...
	new_working_dir = builtin_cd_get_new_working_dir(cmd, env_var_list); // ! MALLOC")
	...
	if (chdir(new_working_dir) != 0)
		return (free(current_working_dir), free(new_working_dir), mini_error_test(error, 1, cmd->args[1]), ERROR); // ! 2x FREE
